...
const SESSION_TTL_MS =
  Number(process.env.SESSION_TTL_MS || 28800000);
...
async function getAgentSessions() {
  ...
  return (
    result?.data?.agentSession?.agentSessions || []
  );
}
...
function getPrimaryChannelInfo(agent) {
  ...
  return (
    channels.find(
      c =>
        String(c.channelType).toLowerCase() ===
        "telephony"
    ) ||
    channels[0] ||
    null
  ...
  async (req, res) => {
    try {
      ...
      const tasks = allTasks
        .filter(
          t =>
            String(t.channelType).toLowerCase() ===
            "telephony"
        )
        .filter(
          t => t?.lastTeam?.id === userTeamId
        );
      ...
      res.json({
        ...
        taskList: tasks.map(task => ({
          ...
          agent:
            task?.lastAgent?.name || "",
          queueDuration:
            task.queueDuration || 0,
          connectedDuration:
            task.connectedDuration || 0
        }))
      });
    } catch (err) {
      res.status(500).json({
...
app.listen(PORT, () => {
  console.log(
    `Secure widget backend listening on ${PORT}`
  );
});
